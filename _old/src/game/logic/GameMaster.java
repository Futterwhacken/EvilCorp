...
import game.logic.config.*;
import game.logic.area.World;
import game.logic.area.region.*;
...
public final class GameMaster extends GameLogic
{
    ...
    private GameMaster(String configPath) { // path relatif à l'endroit où est appelé GameMaster
        ...
        ConfigReader.readConfig(configPath+"config.txt");
        Region[] regions = ConfigReader.readRegions(configPath+"regions.txt");
        EventTriggerable[] eventsT = ConfigReader.readTriggerableEvents(configPath+"events_triggerable.txt");
        EventBuyable[] eventsB = ConfigReader.readBuyableEvents(configPath+"events_buyable.txt");
